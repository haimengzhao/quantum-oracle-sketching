"""IMDB sweep: machine size vs PCA variance recovery.

Top singular direction of the TF-IDF matrix, computed after rare-feature
truncation (min_df) or an oblivious feature sketch, lifted back to the full
feature space and scored by relative explained variance; see sweep_utils for
the shared pipeline.
"""
...
n_sketch_seeds = 5
rare_extra_markers = [-2, -5, -10, -15, -19, -24]
...
def run_analysis(load_file=None, mode=None, n_sketch_seeds=n_sketch_seeds):
    ...
    if load_file is None:
        if sketch_mode:
            ...
            print(f"Running PCA Analysis on {sketch.name}-sketched IMDB Dataset...")
            ...
            load_file = sweep_utils.save_sweep_json(
                ...
                output_json=f"imdb_{mode}_size_vs_variance.json",
                sketch_mode=True,
...
def plot_results(stats, mode, sketch_mode):
    ...
    plt.tight_layout()
    if sketch_mode:
        output_pdf = f"imdb_{mode}_size_vs_variance.pdf"
    else:
...
if __name__ == "__main__":
    sweep_utils.sweep_main("IMDB Machine Size vs Variance Analysis", run_analysis)
